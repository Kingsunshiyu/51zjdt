        ...
        if ($operation == 'post') {
			...
			if ($_GPC['out_put'] == 'output') {
               ...
               foreach($list as $index => $row){
				...
				if ($_W['isfounder']){
					$this->exportexcel($arr, array('订单号','项目名','学生','班级','缴费人','手机号','付费时间','金额','支付状态','支付方式','支付类型','收款公众号'), time());
				}else{
					$this->exportexcel($arr, array('订单号','项目名','学生','班级','缴费人','手机号','付费时间','金额','支付状态','支付方式','支付类型'), time());
				}
                exit();
            }
			////////////////////////////////			
