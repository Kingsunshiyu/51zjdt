 ...
 */
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
				...
				if($_GPC['macname'] ==2){
					if (empty($_GPC['pop']) || empty($_GPC['pic1']) || empty($_GPC['pic2']) || empty($_GPC['pic3']) || empty($_GPC['pic4']) ){
						 message('广告语、图片1-4，不可为空！', $this->createWebUrl('check', array('op' => 'display', 'schoolid' => $schoolid)), 'error');
					}			
					$temp = array(
						'isflow' => 1,
						'pop' => $_GPC['pop'],
				...
				if($_GPC['macname'] ==3){
					if (empty($_GPC['pop1']) || empty($_GPC['pic11']) || empty($_GPC['pic21']) || empty($_GPC['pic31']) || empty($_GPC['pic41']) || empty($_GPC['VOICEPRE1']) ){
						 message('提示语言、滚动公告、图片1-4，不可为空！', $this->createWebUrl('check', array('op' => 'display', 'schoolid' => $schoolid)), 'error');
					}					
					$temp = array(
						'isflow' => 1,
						'pop' => $_GPC['pop1'],
                ...
                message('操作成功！', $this->createWebUrl('check', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
            }
            ...
            if (empty($item)) {
                message('抱歉，不存在或是已经被删除！', $this->createWebUrl('check', array('op' => 'display', 'schoolid' => $schoolid)), 'error');
            }
            pdo_delete($this->table_checkmac, array('id' => $id));
            message('删除成功！', $this->createWebUrl('check', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
