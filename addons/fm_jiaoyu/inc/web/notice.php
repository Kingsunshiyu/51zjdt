 ...
 */
        ...
        if ($operation == 'display') {
				...
				$leave = pdo_fetchall("SELECT * FROM ".tablename($this->table_leave)." WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And isliuyan = 1 And isfrist = 1 $condition ORDER BY createtime DESC, id DESC LIMIT ".($pindex - 1) * $psize.','.$psize, $params);

				foreach ($leave as $key => $row) {
				    if (!empty($row['leaveid'])) {
						$leave[$key]['conet'] = pdo_fetchall("SELECT * FROM " . tablename($this->table_leave) . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' And isliuyan = 1 AND leaveid =:leaveid  ORDER BY createtime DESC LIMIT 0,8 ", array(':leaveid'=>$row['leaveid']));
					}
					//$member = pdo_fetch("SELECT * FROM " . tablename ( 'mc_members' ) . " where uniacid = :uniacid And uid = :uid ", array(':uniacid' => $_W ['uniacid'], ':uid' => $row['tuid']));
					$student = pdo_fetch("SELECT * FROM " . tablename ($this->table_students) . " where weid = :weid And id = :id ", array(':weid' => $_W ['uniacid'], ':id' => $row['sid']));
					$user = pdo_fetch("SELECT * FROM " . tablename ($this->table_user) . " where weid = :weid And schoolid = :schoolid And sid = :sid And uid = :uid ", array(':weid' => $_W ['uniacid'], ':schoolid' => $row['schoolid'], ':schoolid' => $schoolid, ':sid' => $row['sid'], ':uid' => $row['uid']));
					$teacher = pdo_fetch("SELECT * FROM " . tablename ($this->table_teachers) . " where weid = :weid And id = :id ", array(':weid' => $_W ['uniacid'], ':id' => $row['tid']));
					$bj = pdo_fetch("SELECT * FROM " . tablename($this->table_classify) . " WHERE sid = :sid", array(':sid' => $row['bj_id']));
					$leave[$key]['bjname'] = $bj['sname'];
				//	$leave[$key]['tavatar'] = $member['avatar'];
				//	$leave[$key]['tnickname'] = $member['nickname'];
					$leave[$key]['s_name'] = $student['s_name'];
					$leave[$key]['guanxi'] = $user['pard'];
					$leave[$key]['tname'] = $teacher['tname'];
				}
				$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_leave) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And isliuyan = 1 And isfrist = 1 $condition ");
				$pager = pagination($total, $pindex, $psize);
            ...
            message('已经全部清理！',referer(), 'success');
        }
