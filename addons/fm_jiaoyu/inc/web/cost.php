            ...
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
					if (!empty($_GPC['about'])) {
						...
						if (!empty($about)) {   
							message('你选择的关联功能已经其他收费项目中存在,或先停止使用之前已经关联过改功能的收费项目！', '', 'error');
						}else{
					...
					if ($cost['about'] != intval($_GPC['about'])){
						message('错误,缴费项目的关联功能不可更改！', '', 'error');
					}else{	
						pdo_update($this->table_cost, $data, array('id' => $id));
					}
                }
                message('创建付费项目成功！', $this->createWebUrl('cost', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
            }
