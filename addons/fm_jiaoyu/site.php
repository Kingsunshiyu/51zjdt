...
class Fm_jiaoyuModuleSite extends Core {
    ...
    function upload_assess($strs, $time, $array) {
        ...
        $assess = pdo_fetch("SELECT * FROM " . tablename('wx_school_teachers') . " WHERE tname=:tname AND mobile=:mobile AND weid=:weid And schoolid=:schoolid LIMIT 1", array(':tname' => trim($strs[1]), ':mobile' => $strs[4], ':weid' => $_W['uniacid'], ':schoolid'=> $array['schoolid']));
    ...
    function upload_chengji($strs, $time, $array) {
        ...
        $insert = array();
		//名字处理
		$sid = pdo_fetch("SELECT id FROM " . tablename('wx_school_students') . " WHERE s_name=:s_name AND weid=:weid And schoolid=:schoolid ", array(':s_name' => trim($strs[1]), ':weid' => $_W['uniacid'], ':schoolid'=> $array['schoolid']));
		//年级处理
