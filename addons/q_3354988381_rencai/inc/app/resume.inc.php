...
global $_W, $_GPC;
        ...
        if (isset($_GPC['positiontype'])) {
            ...
            if ($_SESSION['weixin_get_fans_location'] && $_SESSION['weixin_get_fans_location'] != '未知') {
        ...
        $time = time();
        //置顶
        $top_lists = pdo_fetchall("SELECT * FROM " . tablename($this->person_table) . " WHERE weid = :weid $where AND istop = 1 AND expiration > {$time} ORDER BY updatetime DESC", array(":weid" => $this->weid));
        ...
        $lists = pdo_fetchall("SELECT * FROM " . tablename($this->person_table) . " WHERE weid = :weid $where AND istop = 0  ORDER BY updatetime DESC limit $begin_rec,$per_page_rec", array(":weid" => $this->weid));
