...
global $_W, $_GPC;
        ...
        $lists = pdo_fetchall("SELECT cmt.*, job.title as job_name,company.name as company_name, fans.nickname, mem.avatar "
                ...
                . "WHERE 1 $condition and cmt.weid = :weid order by cmt.id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(":weid" => $this->weid));
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(*) '
                ...
                . "LEFT JOIN " . tablename('mc_mapping_fans') . " fans ON cmt.from_user=openid "
                . "LEFT JOIN " . tablename('mc_members') . " mem ON fans.uid=mem.uid "                
                . "WHERE 1 $condition and cmt.weid = :weid ", array(":weid" => $this->weid));
