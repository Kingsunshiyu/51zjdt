...
<div id="liveTicketBox" class="main_box_3">
	...
	<div class="ticket_header">
		 <img src="<?php  echo $topic['topic_icon'];?>" style="height:200px;">
		 <?php  if($is_manager) { ?>
		...
		<dl class="ticket_content">
			<dd>
				<span class="ticket_title"><?php  echo $topic['topic_name'];?></span>
			</dd>
			
 			<dd class="distribute_btnbox"> 				
 			</dd> 			
			<dd>
				<span class="ticket_p2 fbold">来自直播间</span><a class="from_live_name " href="<?php  echo $this->createMobileUrl('c_index',array('room_id'=>$chat_room['room_id']))?>"><?php  echo $chat_room['room_name'];?></a>
			    <a class="fromlive_focus " href="javascript:;">关注</a>
			</dd>
			
			<dd style="display:block;">
				<span class="ticket_p2 fbold">主讲人</span>
				<span class="ticket_p1 show_guestName"><?php  echo $topic['guest_name'];?></span>
			</dd>
			<dd style="display:block;">
			...
			<dd class="ticket_people">
				<div class="ticket_people_box">
					<span class="ticket_p1 fbold">已报名人数</span><span class="ticket_p2"><?php  echo $topic['look_numbers'];?>人</span><br>
					<div class="paypeople_imgbox">
					
					<?php  if(is_array($join_list)) { foreach($join_list as $joins) { ?>
					  <span><img src="<?php  echo $joins['avatar'];?>"></span>
					<?php  } } ?>
...
<a class="<?php  if($topic['room_paymoney']>0) { ?>ticket_exit2 payinto_btn<?php  } else { ?>ticket_exit1 manageinto_btn<?php  } ?>" href="<?php  echo $this->createMobileUrl('chat',array('topic_id'=>$topic['id']))?>">
<?php  if($is_join>0) { ?>
   进入直播
<?php  } else { ?>
   报名进入
