...
<div class="order-container">
    ...
    <div class="tab-wrap">
        <ul id="order-tab" class="order-tab hbox">
            <li class="flex <?php  if($_GPC['status']=='all') { ?>cur<?php  } ?>" onclick="location.href='<?php  echo $this->createMobileUrl("mylesson", array("status"=>"all"));?>'">全部</li>
            <li class="flex <?php  if($_GPC['status']=='0') { ?>cur<?php  } ?>" onclick="location.href='<?php  echo $this->createMobileUrl("mylesson", array("status"=>"0"));?>'">待付款</li>
            <li class="flex <?php  if($_GPC['status']=='1') { ?>cur<?php  } ?>" onclick="location.href='<?php  echo $this->createMobileUrl("mylesson", array("status"=>"1"));?>'">已付款</li>
        </ul>
    </div>
...
<div style="font-size:.12rem; text-align: center;<?php  if(empty($mylessonlist)) { ?>display:none;<?php  } ?>">
...
$(function () {
    ...
    function insertDiv(result) {  
        ...
        for (var j = 0; j < result.length; j++) {  
			...
			chtml += '			<span style="display:inline-block;float:right;"><a href="'+payurl+'&orderid='+result[j].id+' " class="pay-btn">支付订单</a></span>';
